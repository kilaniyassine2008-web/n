...
function displaySecretMessage(title, text) {
    ...
    for (let i = 0; i < lines.length; i += 10) {
        secretSlides.push(
            lines.slice(i, i + 10).join('<br>')
        );
    }

    currentSecretSlide = 0;

    const container = document.getElementById('secretSlidesContainer');
    ...
    document.getElementById('secretSlideNumber').innerText =
        `1 / ${secretSlides.length}`;

    document.getElementById('secretMessageModal')
...
function showSecretSlide(index) {
    ...
    document.getElementById("secretSlideNumber").innerText =
        `${index + 1} / ${secretSlides.length}`;
}

function changeSecretSlide(direction) {
    currentSecretSlide =
        (currentSecretSlide + direction + secretSlides.length) %
        secretSlides.length;

    showSecretSlide(currentSecretSlide);
}
